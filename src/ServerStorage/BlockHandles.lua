...
function handles.selectionBox:onHover(part, func, leaveFunc)
	local hover = part:FindFirstChildOfClass("ClickDetector").MouseHoverEnter:Connect(func)
	local hoverEnded = part:FindFirstChildOfClass("ClickDetector").MouseHoverLeave:Connect(leaveFunc)
	
	return hover, hoverEnded
end

function handles.selectionBox:onClick(part, func)
	local click = part:FindFirstChildOfClass("ClickDetector").MouseClick:Connect(func)
